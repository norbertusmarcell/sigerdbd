...
# =========================
# Geocoding & Forecast (DAILY → 7d weekly-like features)
# =========================
...
def fetch_open_meteo_daily(lat: float, lon: float, tz: str, days: int = HORIZON_DAYS):
    ...
    df = pd.DataFrame({
        ...
        "ws_mean": d.get("wind_speed_10m_mean"),
        "ws_max": d.get("wind_speed_10m_max"),
        "tcc_mean": d.get("cloud_cover_mean"),
        "hu_mean_daily": d.get("relative_humidity_2m_mean"),
    })
    return df
...
def summarize_7d_from_daily(df_daily: pd.DataFrame) -> pd.DataFrame:
    ...
    out = pd.DataFrame({
        ...
        "t_max": [float(np.nanmax(pd.to_numeric(d["t_max"], errors="coerce")))],
        "hu_mean": [hu_mean],
        "ws_mean": [float(np.nanmean(pd.to_numeric(d["ws_mean"], errors="coerce")))],
        "tcc_mean": [float(np.nanmean(pd.to_numeric(d["tcc_mean"], errors="coerce")))],
        "rain_hours": [int(np.nansum(pd.to_numeric(d["precipitation_hours"], errors="coerce")))],
        "rain_frac": [float(np.nansum(pd.to_numeric(d["precipitation_hours"], errors="coerce"))) / float(total_hours) if total_hours else 0.0],
...
def get_daily(lat, lon):
    return fetch_open_meteo_daily(lat, lon, tz="Asia/Jakarta", days=HORIZON_DAYS)

df_daily = get_daily(lat, lon)
if df_daily.empty:
    st.error("Open-Meteo tidak mengembalikan data harian untuk lokasi ini.")
    st.stop()

st.write("Cuaca Harian (preview):")
st.dataframe(df_daily.head(HORIZON_DAYS))

df_future_wx = summarize_7d_from_daily(df_daily)
